...
for i in {1..20}; do home=$(curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1/); if [ $home -eq 200 ]; then break; fi; echo "Curling Localhost Faild: $i/20" && sleep 1; done
...
# Test the party page
party=$(curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1/party_1)
...
vote=$(curl -s -o /dev/null -w "%{http_code}" -d "id=964501563&party_name=מרצ" -X POST http://127.0.0.1/vote)
if [ $vote -ne 200 ]; then
...
# Test the login page
login=$(curl -s -o /dev/null -w "%{http_code}" -d "username=admin&password=admin" -X POST http://127.0.0.1/login)
...
add_party=$(curl -s -o /dev/null -w "%{http_code}" -d "party_name=TestParty&party_platform=TestPlatform" -X POST http://127.0.0.1/add-party)
...
delete_party=$(curl -s -o /dev/null -w "%{http_code}" -d "party_name=TestParty" -X POST http://127.0.0.1/delete-party)
