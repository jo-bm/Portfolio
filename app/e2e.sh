...
# Test the home page
home=$(curl -s -o /dev/null -w "%{http_code}" http://0.0.0.0:5000/)
if [ $home -ne 200 ]; then
...
# Test the party page
party=$(curl -s -o /dev/null -w "%{http_code}" http://0.0.0.0:5000/party_1)
...
vote=$(curl -s -o /dev/null -w "%{http_code}" -d "id=964501563&party_name=מרצ" -X POST http://0.0.0.0:5000/vote)
if [ $vote -ne 200 ]; then
...
# Test the login page
login=$(curl -s -o /dev/null -w "%{http_code}" -d "username=admin&password=admin" -X POST http://0.0.0.0:5000/login)
...
add_party=$(curl -s -o /dev/null -w "%{http_code}" -d "party_name=TestParty&party_platform=TestPlatform" -X POST http://0.0.0.0:5000/add-party)
...
delete_party=$(curl -s -o /dev/null -w "%{http_code}" -d "party_name=TestParty" -X POST http://0.0.0.0:5000/delete-party)
